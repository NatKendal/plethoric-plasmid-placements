...
def calcCdfValues(avg=45, std=15, step_length = 5):

    # convert minutes to steps
    min_val = math.floor((avg - 1 * std)/step_length)
...
class MaturationDelayFunctionNormalSkewed(BaseDelayFunction):
    def __init__(self):
        cdf_vals, min_val, max_val = calcCdfValues()
        edge_vals = calcMaturationWeights(cdf_vals)
        edge_vals[max_val] = 1.0
        super().__init__("maturationDelayFunctionNormalSkewed", 2, ["m"], {"maturation_min":min_val, "maturation_max":max_val}, min_val, max_val, edge_vals)
